...
/** Filters and ranks the search index against `query`. Title matches rank above body-only matches; ties keep the index's original (newest-first) order. */
...
export const ReleaseNoteSearchProvider = ({
  ...
  const allMatches = useMemo(() => {
    ...
    return searchIndex(index, trimmedQuery);
...
export const ReleaseNoteSearchResults = ({
  ...
  const { isSearching, isWaitingForIndex, indexError, index, allMatches, trimmedQuery } =
    useReleaseNoteSearchContext();
    ...
    <div className={containerClassName}>
      ...
      {!isWaitingForIndex && index && allMatches.length === 0 && (
        <p className="rn-search-empty-state">
          No release notes match &ldquo;{trimmedQuery}&rdquo;. Try a
          different keyword.
        </p>
      )}
        ...
        <>
          <p className="rn-search-result-count">
            {allMatches.length > MAX_RESULTS
              ? `Showing the ${MAX_RESULTS} most relevant of ${allMatches.length} matching release notes`
