...
function TwoColumnLinks(props) {
  ...
  const processedGroups = columns.map((group, index) => (
    <span
      className={`pds-cta-link-group group-${
        index + 1
      } pds-grid-item pds-grid-item--sm-4 pds-grid-item--md-12 pds-grid-item--lg-6`}
    >
      {columns[index]}
    </span>
  ))
  ...
  return (
    <ul className="two-column-links pds-grid pds-grid--wide pds-spacing-mar-block-end-6xl">
      {processedGroups}
    </ul>
  )
}
