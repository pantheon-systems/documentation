...
- **WordPress:** install a plugin like [Autoptimize](https://wordpress.org/plugins/autoptimize/) to add a similar option in the WordPress admin dashboard. Note, please [use this guide](/modules-plugins-known-issues#autoptimize) to configure the plugin to write files within the standard `wp-content/uploads` path. 
