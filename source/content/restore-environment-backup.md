...
If you want to download a backup using wget, put the provided temporary link in double quotes (`"`) and include the `-O` option to specify the output file and extension based on which backup you are downloading. Replace both the temporary link and `output-file` in this example:
