...
3. Now we need to commit the changes, just like we did earlier. This ensures they are tracked in version control. Changes that aren't committed cannot be deployed to the **<Icon icon="grid" /> Test** environment.
...
4. Now that the changes are committed we can move them over to our **<Icon icon="grid" /> Test** environment with the the **<Icon icon="rotate" /> Deploy** menu.
