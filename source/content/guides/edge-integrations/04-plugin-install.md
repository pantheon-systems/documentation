...
A company using Edge Integration might like to use query strings to define a visitor's interest so that they can run targeted ads and create a site-wide personalized experience for the customer persona. Using our custom wrapper function, `get_geo`, WordPress engineers can check for a set an interest using a query string.

The WordPress SDK repository contains [an example](https://github.com/pantheon-systems/edge-integrations-wordpress-sdk/blob/main/docs/interest.md#example) of how this could be implemented.
