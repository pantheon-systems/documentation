...
1. Since we modified `composer.json` we will need to update Composer. This will also download the defined dependencies:
    ...
    <Image alt="image of terminal running a composer install" src="guides/drupal-8-composer-no-ci/drops-8-composer-update.png" />
...
5. Now let’s run `git status` again to make sure everything is included:
   ...
   <Image alt="Image of git status showing the changed files in red" src="guides/drupal-8-composer-no-ci/drops-8-composer-git-status-after-installing-d8.png" />
...
1. Set the site to `sftp` mode:

    ```bash
    terminus connection:set $PANTHEON_SITE_NAME.dev sftp
    ````
  
2. Use Terminus Drush to install Drupal:
