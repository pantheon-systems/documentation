...
2) To check if it is any of the plugin is breaking the cache, disable the plugins one by one. Do not forget to clear the cache from the Pantheon dashboard everytime that you disable a plugin. The last plugin disabled and made the cache work again most likely is the culprit why the page is not being cached.
