...
- For WordPress, [enable WP_DEBUG & WP_DEBUG_LOG](/guides/logs/faq-logs#how-do-i-enable-error-logging-for-wordpress) in wp-config.php. Then use the debug.log file to find warnings and errors.
...
The **WSOD** is a frustrating issue on WordPress and Drupal sites, since it provides no useful information on the cause. The first place you should look for information is the [log files](/guides/logs). See [PHP Errors and Exceptions](/guides/php/php-errors) for more information on the type of errors you may find.
