...
|**Service / Feature**|**Description**|**Global CDN**|**Advanced Global CDN**|**Advanced Global CDN + WAF/IO<Popover content="Web Application Firewall / Image Optimization" />**|
|--- |--- |--- |--- |--- |
|Price|N/A|Included|Contact Sales|Contact Sales|
|Full-page & Asset Caching|Unlike most CDNs that only handle images and other static assets, full-page technology lets you cache entire pages and assets at the edge.|Yes|Yes|Yes|
|Layer 3 & 4 DDOS protection & mitigation||Yes|Yes|Yes|
|SOC1 & SOC2 Compliance||Yes|Yes|Yes|
|Origin Shielding|Eliminates the ability of attackers to identify IP addresses of origin servers. Value: If the attacker is able to find the IP address, they can bypass traditional CDN protection capabilities. Origin Cloaking prevents these kinds of attacks by hiding your origin from attackers.|Basic|Yes|Yes|
|IP whitelisting & blacklisting|Blacklist - prevent users with specific IPs or in IP ranges from accessing your site. <br/> Whitelist - only allow users with specific IPs or in IP ranges to access your site, blocking everyone else.|Manual Configuration|Yes|Yes|
|Token Authentication / JWT (JSON Web Token)|Use the AGCDN Edge to generate JSON Web Tokens to help build your custom API.|Manual Configuration|Yes|Yes|
|Custom Error & Maintenance page rules||No|Yes|Yes|
|Geolocation based edge control|Take action on the edge based on the physical location of the user (eg. blocking or redirecting based on country) or send back location information to your application for more custom control.|No|Yes|Yes|
|Device detection and edge control||No|Yes|Yes|
|Persistent Presence for continual uptime|Adjusts the 'serve stale content setting '|No|Yes|Yes|
|Full CDN edge logs|See every request to your website, whether or not the content was cached. Full CDN edge logs combine the server logs with logs from the edge to provide a complete picture of your traffic.|No|Yes|Yes|
|Modify headers at the edge|Change request and response headers before your application starts up to create, add, delete, or update parts of your request.|No|Yes|Yes|
|Edge Redirects|Reduce requests on your CMS by moving page redirects to the edge.|No|Yes|Yes|
|Domain Masking and Reverse Proxy|Domain Masking: run multiple Pantheon or non-Pantheon applications behind a single domain, using different URL paths <br/> Reverse Proxy: serve content from another service (eg. S3) seamlessly from your application.|No|Yes|Yes|
|Enterprise Blue/Green support||No|Yes|Yes|
|Efficient Image Optimization (IO) at the edge||No|No|Yes|
|Layer 7/Enterprise DDoS Protection & Mitigation||No|No|Yes|
