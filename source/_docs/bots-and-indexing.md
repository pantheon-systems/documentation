...
The `pantheonsite.io` domains are intended for development use and cannot be used for production. Robots.txt is only visible on Live with a custom domain, and is not available on Dev or Test. Adding sub-domains (i.e. `dev.example.com, test.example.com`) for DEV or TEST  will only remove the `X-Robots-Tag: noindex` header only, but still serve the custom robots.txt from the platform domain.
