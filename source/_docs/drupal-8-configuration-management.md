...
If you prefer to use a GUI to manage configuration management, this is possible by using the Config Direct Save Module, availble on [Drupal.org](https://www.drupal.org/project/config_direct_save).  Activating this module creates a new "Update" menu option under your configuration menu. This module allows you to make backup of configurations (when you check the backup checkbox) and override the sync (all files with old configuration) by the new configurations(replace all configurations files). 
