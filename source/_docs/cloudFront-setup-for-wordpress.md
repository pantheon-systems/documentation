...
- Have an existing WordPress site or [create](/docs/starting-wordpress-site/) one
- Set the site's [connection mode](/docs/developing-directly-with-sftp-mode#sftp-mode) to SFTP
...
There are a few different ways you can install a plugin on a Pantheon hosted WordPress site. The following methods require your site to be in [SFTP Mode](/docs/developing-directly-with-sftp-mode#sftp-mode).

### Install Via Terminus CLI

Execute the following [Terminus](/docs/cli/) command to install and activate the Amazon S3 and CloudFront plugin:
...
Access your WordPress dashboard by appending `/wp-admin` to your [Development Site URL](/docs/create/#visit-the-dev-installation) and log in using your WordPress credentials.
...
2. Add the provided code to the `wp-config.php` file located at the web root of your WordPress site using [SFTP](/docs/developing-directly-with-sftp-mode/):
