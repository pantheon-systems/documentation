...
## Testing Plugins
Adding automated testing is an important step to take for plugins that are being distributed. Automated tests give prospective new users the assurance that the plugin is still working, and also provide a basis for evaluating changes to the plugin.
...
To get started, copy the `require-dev` and `scripts` sections from the composer.json file below into the composer.json of your Terminus plugin.
```
{
    "name": "my-username/terminus-hello-world",
    "description": "A Hello, World Terminus command",
...
- Run `composer install` to install the PHP Code Sniffer.
- Run `composer cs` to check the coding standards of your plugin for PSR-2 compliance.
- Run `composer cbf` to fix PRS-2 compliance errors in your plugin.

The test scripts will create some working files, so you will probably want to add the following content to your plugin's `.gitignore` file:
```bash
vendor
bats
bin
...
Next, define some Bats tests to run. Create a folder named `tests`, and create a file named `confirm-install.bats`. Put the contents below in your Bats test file.
...
```
Replace **MY:PLUGIN-COMMAND** with the name of one of your plugin's commands, and replace **SOME OUTPUT FROM MY PLUGIN HELP** in the test. Then, run your test via:
...
At this point, it would be a good idea to [configure your project tests to run automatically on Circle CI](https://circleci.com/docs/1.0/getting-started/). Copy the contents below into a file named `circle.yml` in your plugin project. Using another testing service can also be done by adapting the contents below; most popular services should be fairly easy to set up.
...
test:
  override:
    - composer test
```
In the Circle CI settings, set up the following environment variables:

- TERMINUS_SITE: The name of a scratch Pantheon site to run tests against.
- TERMINUS_TOKEN: A [Pantheon machine token](https://pantheon.io/docs/machine-tokens/) that has access to the test site.

You will also need to create an ssh key pair, [add the public key to your account on Pantheon](https://pantheon.io/docs/ssh-keys/), and [add the private key to Circle CI](https://circleci.com/docs/1.0/permissions-and-access-during-deployment/) (leave the "Hostname" field empty).
