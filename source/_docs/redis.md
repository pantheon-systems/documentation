...
Currently, all plans except for Personal can use Redis. Redis is available to Sandbox plans for developmental purposes, but Redis will not be available going live on a Personal plan. It is recomended that you do not add Redis to a site with a Personal plan as this will cause unexpected behaviour.
