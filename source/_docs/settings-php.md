...
### Drupal 8
Configure environment-specific settings within the `settings.local.php` file, which is ignored by git in our Drops-8 repository. Modifying the bundled `settings.php` file is not necessary, as it already includes `settings.local.php` if one exists.
...
Drupal 8 will not run without a hash salt so make sure to set one in your `settings.local.php` :
```
$settings['hash_salt'] = 'somerandomstring';
```
