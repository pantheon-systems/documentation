...
If you prefer the command line, you can use [Terminus](/docs/cli) to create backups:  
```
...
![Create site backup Pantheon Dashboard](/source/assets/images/manual-site-backup.png)
...
![Access site backups Pantheon Dashboard](/source/assets/images/direct-download-backup.png)
...
In the event that you need to get your site to a certain point, you can use a [full backup to restore an environment](/docs/restoring-an-environment-from-a-backup).
...
Daily backups are run at a random time during the day. You must have a plan associated with a site to select a specific day for a weekly backup. See [selecting a plan](/docs/selecting-a-plan/) for details about site plans on Pantheon.
...
Additionally, you can manually trigger a full Pantheon backup job for any site environment at any time on your own schedule using [Terminus](/docs/cli/).  Also, you can get S3 download links for retrieval (the links expire and are renewed for additional security).
```bash
