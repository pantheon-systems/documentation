...
For existing sites that already have an active database, you then have to install the (UTF8MB4 Convert)[https://www.drupal.org/project/utf8mb4_convert] module and convert the database. Start by making a backup of the site database, then place the site in maintenance mode. When the coast is clear, run the following command:

```sh
terminus drush <site>.<env> -- utf8mb4-convert-databases
```

This should convert the database tabes in the existing install to the proper encoding to support emoji characters. After making the conversion, test it out by placing an emoji in the site text.
