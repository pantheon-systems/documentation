...
If you are distributing large binaries or hosting big media files, we recommend using a CDN like Amazon S3 as a cost-effective file serving solution that allow uploads directly to S3 from your site without using Pantheon as an intermediary.

 - Drupal sites can use a module such as [Amazon S3 CORS Upload](https://drupal.org/project/amazons3_cors)
 - Wordpress sites can use plugins such as [S3 Uploads](https://github.com/humanmade/S3-Uploads) or [WP Offload S3](https://deliciousbrains.com/wp-offload-s3/)
