...
- Install the [Terminus Composer Plugin](https://github.com/pantheon-systems/terminus-composer-plugin#installation).
...
Once the Pantheon dashboard finishes committing the code, visit your project page on GitHub. Go to your `slogan` pull request. Note that your commit has been added to this pull request, and the Circle CI status indicates that your tests are running. Whenever you commit files from the Pantheon dashboard, the commit will be reduced to contain only those files that belong in the GitHub repository, and this commit will be pushed back to the canonical repository. GitHub will then start a new Circle CI build, and the build results will once again be pushed to the existing multidev environment that was created for this pull request. You may continue working in this environment, making multiple changes, and committing updates whenever you would like your tests to run again.

## Update your Project

When using the Composer workflow, you should never use the Pantheon dashboard to update changes from your upstream, nor should you ever merge code from one environment to another.  All code updates will be done using Composer. Composer commands may be run directly against your Pantheon multidev environments using the Terminus Composer plugin. Using an existing pull request to run a `composer update` command. The example below shows a site that was installed with Drupal 8.3.0, and updated to Drupal 8.3.1 after that version was released.
...
Composer should also always be used to install new modules and themes on your site. Never use the Drupal `Extend` -> `Install new module` feature, nor should you use `drush pm-download`, as neither of these techniques modify the `composer.json` file, which means that modules added this way will disappear the next time the build artifacts are pushed to your Pantheon multidev environment.

In the diagram below, pathauto is installed on a Pantheon multidev environment using the Terminus Composer Plugin. Note that the dependencies of pathauto, token and ctools, are also installed.

![Composer require pathauto](/source/docs/assets/images/pr-workflow/composer-require-pathauto.png)

At this point, you could visit the `Extend` page in the Drupal admin interface to enable pathauto; however, this operation may also be done on the command line using Drush:

![Enable pathauto](/source/docs/assets/images/pr-workflow/enable-pathauto.png)

In Drupal 8, the set of enabled modules is also part of the exportable configuration set. The `Update` tab in the `Configuration Synchronization` section of the Drupal admin interface, demonstrated above, may be used to export this configuration as well. Alternately, this same operation may be done from the command line using Terminus and Drush.

![Drush configuration export](/source/docs/assets/images/pr-workflow/drush-cexy.png)

Commit your changes from the command line with Terminus.

![Terminus commit](/source/docs/assets/images/pr-workflow/terminus-commit.png)

The information needed to install and enable pathauto and its dependencies will then be committed to your GitHub repository; however, the modules sources themselves will not be part of this commit.
