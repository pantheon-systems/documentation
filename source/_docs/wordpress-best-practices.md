...
* In your theme, use a simple PHP `include()` instead of WordPress' [get_template_part()](https://codex.wordpress.org/Function_Reference/get_template_part){.external}. The overhead is heavy if your use case is simply adding in another sub-template file. For example:
