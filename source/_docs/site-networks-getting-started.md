...
Before you begin, set the [connection mode to SFTP](/docs/developing-directly-with-sftp-mode#sftp-mode) for the Development environment. This is required when making necessary modifications to `wp-config.php` through the WordPress Dashboard or via WP-CLI.

### Create a Network Using Terminus and WP-CLI (Recommended)

**Before you begin, make sure that [Terminus](/docs/cli) is configured locally and you’re authenticated with Pantheon.**
...
terminus site hostnames add example-network.com  --env=live
```
Bare domains on live require a [redirect to remove `www.`](/docs/redirect-incoming-requests/#redirect-to-a-common-domain) added to `wp-config.php`, and [special DNS configuration](/docs/domains/#serving-sites-from-bare-domains-with-http).
...
```
Once you feel comfortable with the WordPress Network Dashboard, you’ll be ready to learn how to use the [Pantheon Workflow with WordPress Multisite](/docs/managing/), and pick up a few additional [tips and tricks](/docs/managing#network-tips-and-tricks-with-wp-cli).
