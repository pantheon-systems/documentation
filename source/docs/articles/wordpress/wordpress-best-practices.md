...
* If you have a custom plugin that retrieves a specific post (or posts), instead of using wp_query() to retrieve it, use the get_post() function. While wp_query has it's uses, the [get_post] (https://developer.wordpress.org/reference/functions/get_post/) function is built specifically to retrieve a WordPress Post object, an does so very efficiently. 
