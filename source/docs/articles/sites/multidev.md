...
**Please note**: If you receive an error message that the clone was aborted you should clear the Drupal or WordPress cache in the source environment and retry the procedure. This is because the cache tables can contain entries which exceed the transaction redo limit set by @innodb_log_file_size@.
