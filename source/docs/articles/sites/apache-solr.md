...
First, you will need to add the Index Server to your site. This can be done from the Dashboard by going to Settings > Add Ons > Apache Solr Index Server:
...
- If you have a large number of items to index the process may timeout if you attempt submit them all in one batch. The workaround is to submit a smaller number of items in each batch.
