...
Your site dashboard recommends the specific DNS settings you should use. These can be found under the _Domains /  SSL_ tab, per environment. Under _Domain Setup_, click the link for "Show recommended DNS records" appearing to the right of the domains you've added. In most cases, you'll only need to configure DNS for the _Live_ environment:  
