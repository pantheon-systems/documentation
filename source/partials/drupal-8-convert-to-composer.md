...
1. Copy the old `pantheon.yml` to preserve settings:
 ...
 Remove any values from `pantheon.yml` that you prefer to keep as listed in `pantheon.upstream.yml`. Then add `build_step: true` to `pantheon.yml` if one does not already exist.

 Both `pantheon.yml` and the `api_version: 1` and `build_step: true` values in it are required.
