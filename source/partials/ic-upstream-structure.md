...
When a site is created, Pantheon runs `composer install`, generates a `composer.lock` file, and commits it back to the site’s code repository. To avoid potential merge conflicts after applying an upstream update, do not commit the `composer.lock` file from your upstream root to the upstream repository. Refer to [managing upstream dependencies](/guides/custom-upstream/maintain-custom-upstream#how-to-add-dependencies-to-your-upstream) for more information on how to manage your custom upstream's dependencies.
